...
const fieldClass = 'w-full bg-transparent border-b border-white/60 py-3 text-white placeholder-white/40 text-sm font-light focus:outline-none'
...
export default function Contact() {
  ...
  return (
    <section id="kontakt" style={{ backgroundColor: '#6E2127', color: '#ffffff' }} className="py-20 px-8 md:px-16">

      <h2 className="text-2xl md:text-3xl italic font-normal mb-12">
        Kontakt
      </h2>

      {sent ? (
        <p className="text-sm font-light text-white">Takk for meldingen. Henrik tar kontakt snart.</p>
      ) : (
        <form onSubmit={handleSubmit} className="max-w-2xl grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="md:col-span-2">
            <button type="submit" className="text-white text-sm font-light border-b border-white/60 pb-0.5 hover:opacity-50 transition-opacity">
              Send
            </button>
          </div>
        </form>
